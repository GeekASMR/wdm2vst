...
--*/
{
    ...
    if (m_bCapture)
    {
        // Capture endpoint: IPC bridge provides audio data from VST plugin
        // No tone generator needed - we read from shared memory instead
    }
    else if (!g_DoNotCreateDataFiles)
    {
        // Render endpoint: IPC bridge sends audio data to VST plugin
        // SaveData (file output) not needed - we write to shared memory instead
    }
    ...
    // Initialize IPC bridge with direction prefix and channel ID from endpoint config
    PENDPOINT_MINIPAIR pPair = m_pMiniport->m_pMiniportPair;
    PCWSTR dirPrefix = pPair->IpcIsCapture ? L"REC" : L"PLAY";
    m_IpcBridge.Initialize(dirPrefix, pPair->IpcChannelId);
...
)
{
    ...
    {
        // --- ADDED FIX: Event-Driven DMA Interception ---
        // Push the exact continuous block of audio directly from the payload the OS just generated!
        // This solves brutal DMA buffer tearing/stutter artifacts.
        if (m_ulNotificationsPerBuffer > 0 && !m_bCapture) 
        {
            ULONG numChannels = m_pWfExt->Format.nChannels;
            ULONG bitDepth = m_pWfExt->Format.wBitsPerSample;
            bool isFloat = (m_pWfExt->SubFormat == KSDATAFORMAT_SUBTYPE_IEEE_FLOAT);
            m_IpcBridge.WritePcmToRing(m_pDmaBuffer + ulCurrentWritePosition, packetSize, numChannels, bitDepth, isFloat);
        }
    }
...
)
{
    ...
    else
    {
        ...
        // We ALWAYS call ReadBytes now because it handles IPC bridge to VST!
        // if (!g_DoNotCreateDataFiles)
        // {
            // Read from buffer and write to a file (and IPC).
...
--*/
{
    ...
    while (ByteDisplacement > 0)
    {
        ...
        // This is the Capture endpoint (Microphone). VST -> Driver -> OBS
        // Instead of generating a tone/silence, fetch from VST Shared Memory
        m_IpcBridge.ReadRingToPcm(m_pDmaBuffer + bufferOffset, runWrite, numChannels, bitDepth, isFloat);
...
--*/
{
    ...
    while (ByteDisplacement > 0)
    {
        ...
        // This is the Render endpoint (Speaker). System -> Driver -> VST
        if (m_ulNotificationsPerBuffer == 0) {
            // Timer driven mode (Legacy Fallback)
            m_IpcBridge.WritePcmToRing(m_pDmaBuffer + bufferOffset, runWrite, numChannels, bitDepth, isFloat);
        }
...
)
{
    ...
    if (_this->m_KsState != KSSTATE_RUN || !_this->m_pMiniport)
    {
        goto End;
    }
