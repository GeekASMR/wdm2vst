...
class AsmrtopWdm2VstAudioProcessor : public juce::AudioProcessor, public juce::AudioIODeviceCallback, public juce::Timer
{
public:
    AsmrtopWdm2VstAudioProcessor();
    ~AsmrtopWdm2VstAudioProcessor() override;
    ...
    const juce::String getName() const override { return PLUGIN_W2V_NAME; }
    bool acceptsMidi() const override { return true; }
    bool producesMidi() const override { return false; }
    double getTailLengthSeconds() const override { return 99999.0; }
    bool isBusesLayoutSupported (const BusesLayout& layouts) const override { 
        // LUNA and other strict DAWs might insert this on Mono tracks. 
        // We must accept Mono or Stereo to ensure they don't mute the output bus.
    ...
    void setStateInformation (const void* data, int sizeInBytes) override {
        ...
        if (xmlState != nullptr && xmlState->hasTagName (vts.state.getType())) {
            ...
            if (auto* devSetup = xmlState->getChildByName("DEVICESETUP")) deviceManager.initialise(2, 0, devSetup, false);
    ...
    int getLastNumSamples() const { return lastNumSamples.load(std::memory_order_relaxed); }
...
private:
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AsmrtopWdm2VstAudioProcessor)
};
